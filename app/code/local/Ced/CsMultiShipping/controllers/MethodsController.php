...
class Ced_CsMultiShipping_MethodsController extends Ced_CsMarketplace_Controller_AbstractController
{
    ...
    public function indexAction()
    {
    	...
    	$this->getLayout()->getBlock('head')->setTitle($this->__('Shipping')." ".$this->__('Methods'));
    	$this->renderLayout();
    }
	...
	public function saveAction() {
		...
		if(strlen($section) > 0 && $this->_getSession()->getData('vendor_id') && count($groups)>0) {
			...
			try {
				...
				$this->_getSession()->addSuccess($this->__('The Shipping Methods has been saved.'));
