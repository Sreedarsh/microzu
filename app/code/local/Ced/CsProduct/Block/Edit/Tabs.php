...
class Ced_CsProduct_Block_Edit_Tabs extends Mage_Adminhtml_Block_Widget_Tabs
{
    ...
    protected function _prepareLayout()
    {
        ...
        if ($setId) {
            ...
            //checking advanced form condition
        if(Mage::getStoreConfig('ced_vproducts/general/allow_advanced',Mage::app()->getStore()->getId())){
            ...
            $this->addTab('categories',  array(
                    ...
                    'url'   => Mage::getUrl('*/*/categories', array('_current' => true)),
                    'class' => 'ajax',
             ));
         }
         else
         {
            ...
            $attributes = Mage::getModel('catalog/product_attribute_api')->items($set);
            //$_attribute_code = Mage::getModel('eav/entity_attribute')->load('71');
            //print_r($_attribute_code);die;
            foreach ($attributes as $key => $attribute) {
               ...
               // }
                ...
                $this->addTab('basic', array(
                    'label'     => Mage::helper('catalog')->__('Basic'),
                    'content'   => $this->_translateHtml(Mage::app()->getLayout()->createBlock('csproduct/edit_tab_general','basic')
                            //->setGroup($group)
                            ->setGroupAttributes($attributes)
                            ->toHtml()),
                ));
                ...
                /*$this->addTab('basic',  array(
                    'label'     => Mage::helper('catalog')->__('Basic'),
                    'content'   => $this->_translateHtml($this->getLayout()
                        ->createBlock('adminhtml/catalog_form')->setTemplate('csproduct/edit/tab/general.phtml')->toHtml()),
                ));*/
